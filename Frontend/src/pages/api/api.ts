...
export const returnBook = async (loanId: number, bookId: number, userId: number, return_date: string) => {
  return api.post('/loans/return', {
    loanId,
    bookId,       // Send the book_id
    userId,       // Send the user_id
    return_date,   // Send the return_date
  });
};
